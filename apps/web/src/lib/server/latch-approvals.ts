...
import { z } from "zod";
import { prepareLatchDraft, savedLatchTask, threadMarker } from "./latch-approval-domain";
import type { WorkplaceTask } from "../followup-types";
import type { LatchProposal, LatchSavedTask } from "../latch-approval";
...
export class LatchApprovalService {
  constructor(
    private workplace: Workplace,
    private directory: string,
    private now = Date.now,
  ) {}
  async list(threadId: string): Promise<LatchSavedTask[]> {
    z.string().min(1).max(80).parse(threadId);
    const tag = threadMarker(threadId);
    const tasks = (await this.workplace.list(tag)).filter(t => t.description.split("\n").includes(tag));
    return tasks.map(task => savedLatchTask(task, threadId));
  }
  async get(id: string, threadId: string): Promise<LatchSavedTask> {
    return savedLatchTask(await this.workplace.get(id), threadId);
  }
  async propose(session: string, input: unknown): Promise<LatchProposal> {
    const draft = prepareLatchDraft(input);
  ...
  async approve(session: string, id: string): Promise<LatchSavedTask> {
    ...
    const reconcile = async () => {
      const found = (
        await this.workplace.list(`latch-action:${proposal.actionKey}`)
      ).filter(
        (t) => t.description.split("\n").includes(`latch-action:${proposal.actionKey}`),
      );
      if (found.length > 1)
        throw new FollowupError(
          "Ambiguous contains multiple matching records; inspect the workspace before continuing.",
        );
      if (found[0] && (found[0].title !== proposal.title || found[0].description !== proposal.description))
        throw new FollowupError("This commitment already has a saved record with different reviewed fields. Refresh to inspect it; editing a title cannot create another copy.");
      return found[0];
    };
    const readBack = async (id: string) => {
      ...
      return savedLatchTask(record, proposal.threadId);
    };
